...
class ScisTree2():
    ...
    def evaluate_cupy(geno, nwk, offset=-1):
        ...
        geno = cp.array(geno)
        print(geno.shape)
        max_mls = cp.zeros(geno.shape[0]) - cp.inf 
        max_ml_nodes = [None] * geno.shape[0]
        g = cp.log(1-geno) - cp.log(geno) # in log space to avoid numerical overflow
        for node in traveror(tree):
            if node.is_leaf():
                likelihood = g[:, int(node.name)]
            else:
            ...
            print(1)
        # print(max_ml_nodes)
        max_mls += cp.log(geno).sum(axis=1)
        imputed_genotype = cp.zeros_like(geno, dtype=int)
        # print(imputed_genotype.sum(axis=-1))
        for i, ml_node in enumerate(max_ml_nodes):
            inds = [int(leaf.name) for leaf in ml_node.get_leaves()]
            imputed_genotype[i, inds] = 1
